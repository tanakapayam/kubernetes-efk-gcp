...
# This is only necessary to run _once_ at cluster create time. After the first GCE volume is created,
...
if [ -d "$vol" -a "$(stat -c '%U' "$vol" 2>/dev/null)" = "root" ]; then
    chown -R elasticsearch:elasticsearch "$vol"
    chmod o-rwx "$vol"
fi

sudo -E -u elasticsearch bash -c "/usr/share/elasticsearch/bin/elasticsearch"
